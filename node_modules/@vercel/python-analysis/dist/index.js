...
var readFileStorage = new AsyncLocalStorage();
...
function stripWorkingDir(path4, workingDir) {
  const normalized = normalize(path4);
  if (!workingDir) {
    return normalized;
  }
...
var uvConfigSchema = z3.object({
  ...
  "dev-dependencies": z3.array(z3.string()).optional()
});
...
var dependencyGroupIncludeSchema = z4.object({
  "include-group": z4.string()
});
var dependencyGroupEntrySchema = z4.union([
  z4.string(),
  dependencyGroupIncludeSchema
]);
var pyProjectDependencyGroupsSchema = z4.record(
  z4.array(dependencyGroupEntrySchema)
);
...
import { isAbsolute, join as join3, relative } from "path/posix";
...
async function maybeLoadRequirementsTxt(root, subdir, fileName) {
  ...
  try {
    const pyproject = await convertRequirementsToPyprojectToml(
      requirementsContent,
      {
        workingDir: path3.join(root, subdir),
        packageRoot: root
      }
    );
    return {
